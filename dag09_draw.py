    ...
    for [x1,y1],[x2,y2] in zip(corners,corners[1:]):
        ...
        #    |          | |
            ...
            if (y1<y2 and prev_dir=='up') or (y1>y2 and prev_dir=='down'):
                ...
                prev_dir=''
                continue
            if (y1>y2 and prev_dir=='down') or (y1<y2 and prev_dir=='up'):
                #Lijn gaat niet door in dezelfde richting na de knik, gebied links is dus fout
                count+=1
                prev_dir=''
                continue
            count+=1
            if y1<y2 and prev_dir=='':
                prev_dir='up'
            elif y1>y2 and prev_dir=='':
                prev_dir='down'
...
for ind1,[a,b] in enumerate(corners):
    for ind2,[c,d] in enumerate(corners[ind1+1:]):
        ...
        xs2=[d[0] for d in corners]
        ys2=[d[1] for d in corners]
        plt.plot(xs2,ys2,'b-')
        plt.plot(a,b,'b-',marker='o',markersize=5)
        plt.plot(c,d,'b.',marker='o',markersize=5)
        titles='Max:'+str(p2)+' current: '+str(area)+','

        if in_contour(a,d,corners):
            plt.plot(a,d,'g*')
            titles+=' p3 good,'
        else:
            plt.plot(a,d,'r*')
            titles+=' p3 false,'
        if in_contour(c,b,corners):
            plt.plot(c,b,'g*')
            titles+=' p4 good,'            
        else:
            plt.plot(c,b,'r*')
            titles+=' p4 false,' 
        xs1=[d[0] for d in rectangle]
        ys1=[d[1] for d in rectangle]
        if no_intersect(rectangle,corners):
            plt.plot(xs1,ys1,'g:')
            titles+=' no intersects'
        else:
            plt.plot(xs1,ys1,'r:')
            titles+=' intersects found'
        plt.title(titles)
        plt.show()
